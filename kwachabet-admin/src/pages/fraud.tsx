...
export default function FraudPage() {
  ...
  async function resolveFlag() {
    ...
    try {
      await adminAPI.resolveFraudFlag(selected.id, notes);
  ...
  const filteredFlags = flags.filter(f => !search || f.full_name?.toLowerCase().includes(search.toLowerCase()) || f.phone?.includes(search));
  const filteredRisk  = highRisk.filter(u => !search || u.phone?.includes(search) || u.full_name?.toLowerCase().includes(search.toLowerCase()));
    ...
    <>
      ...
      <AdminLayout title="Fraud Detection & Risk Engine">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          <StatCard label="Open Flags"    value={flags.length}     icon="🚨" color={flags.length > 0 ? 'red' : 'green'}     format="number" />
          <StatCard label="Critical"      value={critical}         icon="🔴" color={critical > 0 ? 'red' : 'green'}          format="number" />
          <StatCard label="High Risk"     value={high}             icon="🟠" color={high > 0 ? 'yellow' : 'green'}           format="number" />
          <StatCard label="Risky Users"   value={highRisk.length}  icon="👤" color={highRisk.length > 0 ? 'yellow' : 'green'} format="number" />
        </div>
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={() => setTab('flags')} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${tab === 'flags' ? 'bg-brand text-black' : 'bg-admin-card border border-admin-border text-gray-400'}`}>
            🚨 Fraud Flags ({flags.length})
          </button>
          <button onClick={() => setTab('risk')} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${tab === 'risk' ? 'bg-brand text-black' : 'bg-admin-card border border-admin-border text-gray-400'}`}>
          ...
          <div className="space-y-3">
            {loading ? <TableSkeleton rows={4} cols={1} /> :
             filteredFlags.length === 0 ? (
               <div className="admin-card"><EmptyState icon="🎉" title="No active fraud flags" /></div>
               ...
               <div key={flag.id} className={`admin-card p-4 ${flag.severity === 'critical' ? 'border-red-700/50' : flag.severity === 'high' ? 'border-orange-700/30' : ''}`}>
                 <div className="flex items-start justify-between gap-4">
                   ...
                   <div className="flex flex-col gap-2 flex-shrink-0">
                     <button onClick={() => setSelected(flag)} className="text-xs border border-brand/40 text-brand px-3 py-1.5 rounded-lg hover:bg-brand/10 transition-colors">Resolve</button>
                     <button onClick={() => suspendUser(flag.user_id, flag.full_name)} className="text-xs border border-red-800 text-red-400 px-3 py-1.5 rounded-lg hover:bg-red-900/20 transition-colors">Suspend</button>
                   </div>
                 </div>
               </div>
             ))}
        ...
        {tab === 'risk' && (
          <div className="admin-card overflow-hidden">
            {loading ? <TableSkeleton rows={5} cols={5} /> :
             filteredRisk.length === 0 ? <EmptyState icon="✅" title="No high risk users" /> : (
               <div className="overflow-x-auto">
                 <table className="admin-table">
                   <thead><tr><th>Customer</th><th>Risk Score</th><th>Risk Level</th><th>Status</th><th>Joined</th><th>Actions</th></tr></thead>
                   <tbody>
                     {filteredRisk.map(u => (
                       <tr key={u.id}>
                         <td><p className="text-white text-sm">{u.full_name}</p><p className="text-gray-600 text-xs font-mono">{u.phone}</p></td>
                         <td>
                           <div className="flex items-center gap-2">
                             <div className="flex-1 bg-admin-border rounded-full h-1.5 max-w-16">
                               <div className={`h-1.5 rounded-full ${u.risk_score >= 70 ? 'bg-red-400' : u.risk_score >= 40 ? 'bg-yellow-400' : 'bg-green-400'}`} style={{ width: `${u.risk_score}%` }} />
                             </div>
                             <span className={`text-xs font-bold ${u.risk_score >= 70 ? 'text-red-400' : u.risk_score >= 40 ? 'text-yellow-400' : 'text-green-400'}`}>{u.risk_score}</span>
                           </div>
                         </td>
                         ...
                         <td>
                           {!u.is_suspended && (
                             <button onClick={() => suspendUser(u.id, u.full_name)} className="text-xs text-red-400 hover:text-red-300 px-2 py-1 rounded hover:bg-red-900/20">Suspend</button>
                           )}
                         </td>
                       </tr>
                     ))}
          ...
          {selected && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} className="admin-input resize-none" placeholder="Describe your findings and resolution..." />
              </div>
              <div className="flex gap-2">
                <button onClick={resolveFlag} disabled={actionLoading} className="btn-primary flex-1">{actionLoading ? 'Resolving...' : '✓ Mark Resolved'}</button>
                <button onClick={() => suspendUser(selected.user_id, selected.full_name)} className="btn-danger flex-1">⛔ Suspend User</button>
              </div>
