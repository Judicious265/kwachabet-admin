...
export default function DashboardPage() {
 ...
 useEffect(() => {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [sR, tR, txR, uR] = await Promise.allSettled([
      ...
      if (tR.status === 'fulfilled')  setRecentTickets(tR.value.data.tickets || []);
      if (txR.status === 'fulfilled') setRecentTxns(txR.value.data.transactions || []);
      if (uR.status === 'fulfilled')  setRecentUsers(uR.value.data.users || []);
      setLastUpdated(new Date());
    } catch { toast.error('Failed to refresh dashboard'); }
    finally { setLoading(false); }
  }, []);

  const stakeData = recentTickets.map((t: any) => ({
      ...
      <AdminLayout title="Dashboard">
        ...
        {loading ? <StatsSkeleton /> : stats ? (
          ...
          <div className="admin-card p-6 text-center mb-6">
            <p className="text-red-400 text-sm">Could not load stats. <button onClick={loadAll} className="text-brand underline">Retry</button></p>
          </div>
        )}
                ...
                {recentTickets.map((t: any) => (
                  <div key={t.id} className="px-4 py-2.5 flex items-center justify-between hover:bg-admin-hover transition-colors">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${t.status === 'won' ? 'bg-green-900/40 text-green-400' : t.status === 'lost' ? 'bg-red-900/40 text-red-400' : 'bg-yellow-900/40 text-yellow-400'}`}>
                        {t.status}
                      </span>
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            <div className="admin-card">
              <div className="px-4 py-3 border-b border-admin-border flex items-center justify-between">
                ...
                <a href="/customers" className="text-brand text-xs">All →</a>
              </div>
              {loading ? (
                <div className="p-4 space-y-2">{[...Array(3)].map((_,i) => <Skeleton key={i} className="h-10 w-full" />)}</div>
              ) : recentUsers.length === 0 ? (
                <div className="p-4 text-center"><p className="text-gray-600 text-xs">No users yet</p></div>
              ) : (
                <div className="divide-y divide-admin-border">
                  {recentUsers.map((u: any) => (
                    <div key={u.id} className="px-4 py-2.5 flex items-center gap-2.5 hover:bg-admin-hover transition-colors">
                      ...
                      <p className="text-green-400 text-xs font-bold">{fmt.mwk(u.balance || 0)}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <a key={action.label} href={action.href}
                    className={`flex flex-col items-center justify-center gap-1 p-3 rounded-xl border text-xs font-medium transition-all cursor-pointer text-center ${action.urgent ? 'border-red-700/50 bg-red-900/10 text-red-400 hover:bg-red-900/20' : 'border-admin-border text-gray-400 hover:border-brand/30 hover:text-white hover:bg-admin-hover'}`}>
